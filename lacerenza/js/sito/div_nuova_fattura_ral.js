...
$(document).ready(function() {
	...
	$("#btn_elimina_ral").click(function () {
		...
		var id_fattura_ral = $(this).attr("id_fattura_ral");
		var id_commessa = $(this).attr("id_commessa");
		$("#id_da_eliminare").val(id_fattura_ral);
		$("#id_commessa_da_eliminare").val(id_commessa);
		$("#id_ral_da_eliminare").val(id_ral);
	...
	$("#btn_elimina_conferma").click(function(){
		...
		var id_ral = $("#id_ral_da_eliminare").val();
		var nome = $("#nome_da_eliminare").val();

		$.ajax({
			url: "lib/operazioni_fattura_ral.lib.php",
			type: 'POST',
			data: { tipo: "elimina_allegato", id: id, nome: nome, id_commessa: id_commessa, id_ral: id_ral },
		...
		success: function(data){
		    ...
		    if (tipo != "modifica") {
			    if (data >= 0) {
					...
					$("#titolo_h1").html("Modifica Fattura SAL");
					$("#div_nuova_fattura_ral").load("php/div_nuova_fattura_ral.php?id=" + data + "&id_commessa=" + id_commessa + "&id_ral=" + id_ral);
					$("#ul_log").load("php/ul_log.php");	
				}  else {
					alert("Errore: "+data);	
				}
			    ...
			    if (data >= 0) {
				    var id = $("#id_fattura_ral_modifica").val();
					$("#contenuto_messaggio").html("<img src='img/caricato.png' style='width:20px; margin-right:10px' />Salvataggio avvenuto con successo.");
					$("#messaggio_errore").hide();
					$("#messaggio").show();
					//NASCONDO MESSAGGIO
