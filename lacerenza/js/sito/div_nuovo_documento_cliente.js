...
$(document).ready(function() {
	...
	$("#btn_elimina_documento_cliente").click(function () {
		...
		var id_documento_cliente = $(this).attr("id_documento_cliente");
		var id_commessa = $(this).attr("id_commessa");
		
		$("#id_da_eliminare").val(id_documento_cliente);
		...
		success: function(data){
			...
			if(tipo != "modifica"){
			    if (data >= 0) {
					...
					$("#btn_nuovo_documento_cliente").hide();	
					$("#btn_modifica_documento_cliente").show();
					$("#titolo_h1").html("Modifica Fattura");
					$("#div_nuovo_documento_cliente").load("php/div_nuovo_documento_cliente.php?id=" + data + "&id_commessa=" + id_commessa);
					$("#ul_log").load("php/ul_log.php");	
				}  else {
					alert("Errore: "+data);	
				}
				...
				if(data >= 0){
				    var id = $("#id_documento_cliente_modifica").val();
					$("#contenuto_messaggio").html("<img src='img/caricato.png' style='width:20px; margin-right:10px' />Salvataggio avvenuto con successo.");
					$("#messaggio_errore").hide();
					$("#messaggio").show();
					//NASCONDO MESSAGGIO
					...
					$("#btn_nuovo_documento_cliente").hide();	
					$("#btn_modifica_documento_cliente").show();	
					$("#titolo_h1").html("Modifica Documento");
					$("#div_nuovo_documento_cliente").load("php/div_nuovo_documento_cliente.php?id=" + id + "&id_commessa=" + id_commessa);
					$("#ul_log").load("php/ul_log.php");	
				}  else {
					alert("Errore: "+data);	
