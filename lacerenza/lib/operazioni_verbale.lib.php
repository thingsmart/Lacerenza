...
switch($tipo){
	case "inserimento":
		...
		$verbale = new Verbali();
		$e_query_inserimento = $verbale->inserisciVerbale($id_commessa, $descrizione, $importo, $data,  $target_path_inserimento, $filename);
		
		//LOG
		$log->inserisciLog("Inserimento verbale", $_SESSION['username'], "verde");
	...
	case "elimina":
		...
		$id_commessa = $_POST['id_commessa'];
		$nome = $_POST['nome'];
		$e_query_elimina = $verbale->eliminaVerbale($id);
		if($e_query_elimina > 0){
			if($nome != ""){
				$target_path = "../uploads/commesse/".$id_commessa."/verbali/".$nome;
		...
		//LOG
		$log->inserisciLog("Eliminazione verbale", $_SESSION['username'], "rosso");
	...
	case "elimina_allegato":
		...
		$id_commessa = $_POST['id_commessa'];
		$nome = $_POST['nome'];
		$e_query_elimina = $verbale->eliminaAllegato($id);
		if($e_query_elimina >= 0){
	...
	case "modifica":
		...
		$verbale = new Verbali();
		$e_query_inserimento = $verbale->modificaVerbale($id, $id_commessa, $descrizione, $importo, $data,  $target_path_modifica, $filename);
		
		$log->inserisciLog("Modifica verbale", $_SESSION['username'], "blu");
		echo $e_query_inserimento;
	break;	
}	
