...
switch($tipo){
	case "inserimento":
		...
		$riserva = new Riserve();
		$e_query_inserimento = $riserva->inserisciRiserva($id_commessa, $descrizione, $dettagli, $data, $target_path_inserimento, $filename);
		
		//LOG
		$log->inserisciLog("Inserimento riserva", $_SESSION['username'], "verde");
	...
	case "elimina":
		...
		$id_commessa = $_POST['id_commessa'];
		$nome = $_POST['nome'];
		$e_query_elimina = $riserva->eliminaRiserva($id);
		if($e_query_elimina > 0){
			if($nome != ""){
				$target_path = "../uploads/commesse/".$id_commessa."/riserve/".$nome;
		...
		//LOG
		$log->inserisciLog("Eliminazione riserva", $_SESSION['username'], "rosso");
	...
	case "elimina_allegato":
		...
		$id_commessa = $_POST['id_commessa'];
		$nome = $_POST['nome'];
		$e_query_elimina = $riserva->eliminaAllegato($id);
		if($e_query_elimina >= 0){
	...
	case "modifica":
		...
		$riserva = new Riserve();
		$e_query_inserimento = $riserva->modificaRiserva($id, $id_commessa, $descrizione, $dettagli, $data, $target_path_modifica, $filename);
		
		$log->inserisciLog("Modifica riserva", $_SESSION['username'], "blu");
		echo $e_query_inserimento;
	break;	
}	
