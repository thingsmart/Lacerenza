...
$filtro = isset($_GET['filtro_regolarita']) ? $_GET['filtro_regolarita'] : "";
...
if($filtro == ""){
$e_query_riserva = $regolarita->caricaRegolarita($id_commessa);
} else {
    $e_query_riserva = $regolarita->filtraRegolarita($filtro, $id_commessa);
}
$numeroRegolarita = $regolarita->numeroRegolarita();
?>

<!--SCRIPT SITO-->
<script src="js/sito/tabella_regolarita.js" type="text/javascript"></script>

<? if($numeroRegolarita > 0){ ?>
        ...
        <tbody>
           ...
           $i=0;
			...
			<tr>
				...
				<td style="text-align:center" data-title="Ente">
                    <? if($row['ente'] != ""){?>
                        <?=$row['ente']?>
                    <? } else { ?>
                        Nessuno
                ...
                <td data-title="Modifica" style="text-align:center">
                 	<a style="width:100%" class="btn" href="nuova_regolarita.php?id=<?=$row['id']?>&id_commessa=<?=$id_commessa?>"><i class="fa fa-edit fa-lg"></i></a>
				</td>
                <td data-title="Elimina" style="text-align:center">
                 	<a style="width:100%" class="btn btn_elimina_regolarita" nome="<?=$row['nome_allegato']?>" id="<?=$row['id']?>" data-toggle="modal" data-target=".bs-elimina"><i class="fa fa-trash-o fa-lg"></i></a>
				</td>
			</tr>
			<?php
				} //END WHILE
